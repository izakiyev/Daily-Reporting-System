...
class DatabaseEngine:
    ...
    def get_tasks(self, date_filter: str = None) -> List[TaskItem]:
        """Fetches only THIS employee's tasks from the central H: drive file."""
        user_key = os.getlogin() # This ensures employees only see their own work
    ...
    def get_tasks(self, date_filter: str = None) -> List[TaskItem]:
        user_key = os.getlogin() # Permanent ID
        with self.connection() as conn:
            query = "SELECT * FROM tasks WHERE os_user = ?"
            params = [user_key]
        ...
        try:
            ...
            for t in tasks:
                ...
                evening_data.append({
                    ...
                    "% Completed": f"{t.percent_complete}%",
...
class DateSelectorDialog(QDialog):
    """A custom dialog to select a specific date to import tasks from."""
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Select Source Date")
        self.setFixedWidth(350)
        
        layout = QVBoxLayout(self)
        
        header = HeaderLabel("Import Tasks From...", 14)
        layout.addWidget(header)
        
        self.calendar = QDateEdit(calendarPopup=True)
        # Default to yesterday
        self.calendar.setDate(QDate.currentDate().addDays(-1)) 
        self.calendar.setMinimumHeight(40)
        ...
        self.btn_import = PrimaryButton("Import Selected Date")
...
class MorningTab(QWidget):
    """
    ENT-Grade Morning Planning Terminal.
    Handles task creation, historical data pasting, and deployment dispatch.
    """
    def __init__(self, db: DatabaseEngine, thread_pool: QThreadPool, main_window): 
        super().__init__()
        self.db = db
        self.thread_pool = thread_pool
        self.main_window = main_window  # Critical fix: Store the MainWindow reference
    ...
    def setup_ui(self):
        ...
        self.time_picker.setTime(QTime(17, 0))
        ...
        self.table.setHorizontalHeaderLabels(["ID", "Description", "Priority", "Start Time", "Status"])
    ...
    def refresh_data(self):
        """Syncs the UI with the database using the SELECTED date context."""
        p = self.db.get_profile()
        ...
        self.health_bar.setValue(int((accomplished / total) * 100) if total > 0 else 0)
    ...
    def load_task_to_edit(self, item):
        ...
        if task:
            ...
            self.time_picker.setTime(QTime.fromString(task.planned_time, "HH:mm"))
    ...
    def paste_previous_plan(self):
        """Opens a dialog to choose a specific date and imports tasks from it."""
        # 1. Show the date selector
        dlg = DateSelectorDialog(self)
        if dlg.exec() != QDialog.DialogCode.Accepted:
            return

        source_date = dlg.get_selected_date_iso()
        current_sel_date = self.get_context_date() 

        # Validation: Don't import from the same day
        if source_date == current_sel_date:
            QMessageBox.warning(self, "Invalid Selection", "Cannot import tasks from the same day.")
            return

        # 2. Fetch tasks for that specific date
        source_tasks = self.db.get_tasks(source_date)

        if not source_tasks:
            QMessageBox.information(self, "No Data", f"No tasks found for {source_date}.")
            return

        # 3. Confirm with user
        msg = f"Found {len(source_tasks)} tasks on {source_date}. Import them into your {current_sel_date} plan?"
        if QMessageBox.question(self, "Confirm Import", msg) == QMessageBox.StandardButton.No:
            return

        # 4. Import logic with duplicate prevention
        ...
        for old_t in source_tasks:
            ...
            new_task = TaskItem(
                ...
                morning_notes=f"Imported from {source_date}", # Traceability
                status="Planned"
            )
        ...
        self.refresh_data()
        QMessageBox.information(self, "Import Success", f"Imported {added_count} tasks from {source_date}.")
...
class DashboardTab(QWidget):
    ...
    def setup_ui(self):
        ...
        self.btn_png = QPushButton(" 📸 Export Dashboard PNG ")
        self.btn_png.setCursor(Qt.CursorShape.PointingHandCursor)
        self.btn_png.setMinimumHeight(45)
        self.btn_png.setStyleSheet(f"""
            QPushButton {{ 
                background: white; border: 1px solid {Colors.BORDER}; 
                border-radius: 8px; font-weight: 800; color: {Colors.SECONDARY};
                padding: 0 20px;
            }}
            QPushButton:hover {{ background: #f1f5f9; border-color: {Colors.PRIMARY}; }}
        """)
